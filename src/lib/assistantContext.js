...
const FEEDS = {
  ...
  reComp: "/api/re-composite", rePipe: "/api/re-pipeline", redfin: "/api/redfin", creCredit: "/api/cre-credit", pulse: "/api/us-pulse", intl: "/api/intl-pulse", machine: "/api/machine", dam: "/api/damodaran-erp", commod: "/api/commodity-pulse", ai: "/api/ai-pulse", sfc: "/api/sfc", bk: "/api/bankruptcy", sea: "/api/seattle",
};
...
export function buildVerdictText(d) {
  ...
  if (d.sea?.scores) {
    const m = d.sea, sc = m.scores, U = m.labor.unemployment, P = m.labor.payrolls, J = m.labor.postings, cs = m.housing.metro?.caseShiller, Z = m.housing.metro?.zillow, I = m.housing.inventory, Pr = m.prices, W = m.business.warn, G = m.giants;
    L.push(`SEATTLE METRO (Seattle-Tacoma-Bellevue MSA; BLS/BEA/Census via FRED, Indeed, Zillow/Case-Shiller, WA ESD WARN, W.D. Wash. bankruptcies, Yahoo):`);
    ...
    L.push(`  Labor (${U.msaD?.slice(0, 7)}): unemployment metro ${U.msa}% (King ${U.king}%, Pierce ${U.pierce}%, Snohomish ${U.snoh}%; WA ${U.wa}%, US ${U.us}%), ${pct(U.msaChg1y, 1)}pp yoy, ${U.msaPct}th percentile since 1994. Payrolls ${(P.total / 1e3).toFixed(3)}M, ${pct(P.yoy, 1)} yoy (US ${pct(P.yoyUs, 1)}), ${pct(P.sinceFeb2020, 1)} vs Feb 2020; sectors yoy: ${P.sectors.map(x => `${x.label} ${pct(x.yoy, 1)} (${x.share}%)`).join(', ')}. Avg hourly earnings $${m.labor.earnings.ahe} (${pct(m.labor.earnings.aheYoy, 1)}; US $${m.labor.earnings.aheUs}). WA initial claims 4-wk ${m.labor.claims.wa4w} (${pct(m.labor.claims.wa4wYoy, 1)} yoy). Indeed postings Seattle ${J.sea} vs US ${J.us} (Feb 2020 = 100), ${pct(J.seaChg1y, 1)} yoy.`);
    ...
    L.push(`  Prices: Seattle CPI ${pct(Pr.cpi.sea, 1)} yoy vs US ${pct(Pr.cpi.usAtSameMonth, 1)}; rent CPI ${pct(Pr.rent.sea, 1)} vs ${pct(Pr.rent.usAtSameMonth, 1)}; gasoline $${Pr.gas.sea} vs US $${Pr.gas.us}; regional price parity ${Pr.rpp.v}.`);
    L.push(`  Business: WARN notices ${W.puget90.workers.toLocaleString()} Puget Sound workers in ${W.puget90.notices} notices over 90 days (prior 90: ${W.puget90prev.workers.toLocaleString()}); latest: ${W.notices.slice(0, 6).map(n => `${n.company} ${n.workers} (${n.location}, ${n.received})`).join('; ')}. WA business applications ${m.business.apps.m12?.toLocaleString()}/mo (${pct(m.business.apps.m12Yoy, 1)}).${m.business.bk ? ` W.D. Wash. bankruptcies ${m.business.bk.total} last quarter (${pct(m.business.bk.yoy, 1)}), business ch.11 ${m.business.bk.bizCh11T4} in a year (p${m.business.bk.bizCh11Pct}).` : ''}`);
    L.push(`  Growth (annual, lagged): ${m.growth.map(g => `${g.label} ${g.unit === '$' ? '$' + Math.round(g.v).toLocaleString() : g.unit.startsWith('$k') ? '$' + (g.v / 1e6).toFixed(1) + 'B' : g.unit === 'k' ? (g.v / 1e3).toFixed(2) + 'M' : g.v.toFixed(1)} (${g.d?.slice(0, 4)}, ${g.unit === '%' || g.unit === 'ratio' || g.unit === 'index' ? (g.yoy >= 0 ? '+' : '') + g.yoy : pct(g.yoy, 1)})`).join('; ')}.`);
    L.push(`  Puget Sound 12 (equal weight): ${pct(G.ewYtd, 1)} YTD vs SPY ${pct(G.spyYtd, 1)}; ${G.rows.filter(r => r.sym !== 'SPY').map(r => `${r.sym} ${pct(r.ytd, 1)} YTD`).join(', ')}.`);
  }
  if (d.bk?.scores) {
