...
import { getFreshness, toneColor } from "../lib/dataHealth.js";
...
function RateCard({ label, value, flag, color, subtitle, format, small, date }) {
  ...
  const dateTone = toneColor[freshness.tone] || toneColor.unknown;
  return (
    <div style={{ background: cardBg, border: cardBorder, borderRadius: 14, padding: small ? "12px 14px" : "16px 20px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: small ? 10 : 11, color: "var(--text-secondary)", fontFamily: fonts.mono, letterSpacing: 0.5, textTransform: "uppercase" }}>{label}</span>
      </div>
      <div style={{ fontSize: small ? 22 : 28, fontWeight: 700, color: "var(--text-primary)", fontFamily: fonts.heading, letterSpacing: -0.5 }}>{disp()}</div>
      {(subtitle || date) && <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 3, fontFamily: fonts.mono }}>{subtitle}{subtitle && date ? " | " : ""}{date && <span style={{ color: dateTone }}>{fmtDate(date)}</span>}</div>}
    </div>
  );
}
...
function ChartCard({ data, series, title, height = 200, yFormatter, refLine }) {
  ...
  return (
    <div style={{ background: cardBg, border: cardBorder, borderRadius: 14, padding: "16px 16px 8px 6px" }}>
      <div style={{ fontSize: 11, color: "var(--text-secondary)", fontFamily: fonts.mono, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 10, paddingLeft: 12 }}>{title}</div>
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data} margin={{ top: 5, right: 8, left: -10, bottom: 0 }}>
          ...
          <YAxis tick={{ fill: "var(--text-muted)", fontSize: 9, fontFamily: fonts.mono }} axisLine={false} tickLine={false} domain={[dMin => { const pad = Math.max(Math.abs(dMin) * 0.1, 0.5); return Math.floor((dMin - pad) * 10) / 10; }, dMax => { const pad = Math.max(Math.abs(dMax) * 0.1, 0.5); return Math.ceil((dMax + pad) * 10) / 10; }]} tickFormatter={yFormatter || (v => `${v}%`)} />
          <Tooltip contentStyle={{ background: "var(--tooltip-bg, #0f172a)", border: "1px solid var(--border-subtle)", borderRadius: 8, fontSize: 11, fontFamily: fonts.heading }} labelStyle={{ color: "var(--text-secondary)" }} labelFormatter={fmtDate} formatter={(v, n) => [yFormatter ? yFormatter(v) : `${v.toFixed(2)}%`, n]} />
          <Legend wrapperStyle={{ fontSize: 10, fontFamily: fonts.heading, paddingTop: 6 }} iconType="circle" iconSize={7} />
          {refLine !== undefined && <ReferenceLine y={refLine} stroke="var(--border-subtle)" strokeDasharray="4 4" />}
